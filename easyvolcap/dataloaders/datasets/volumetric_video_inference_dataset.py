...
class VolumetricVideoInferenceDataset(VolumetricVideoDataset):
    def __init__(self,
        ...
        # Apply ratio to camera path intrinsics (load_interpolations overwrites the
        # ratio-scaled values from load_bytes when using custom camera_path files)
        if self.camera_path_intri and self.camera_path_extri:
            self.Ks = self.Ks.clone()
