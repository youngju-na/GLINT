...
class VolumetricVideoSupervisor(VolumetricVideoModule):
    def __init__(self,
                 ...
                 perc_loss_every_n_iter: int = 1,  # compute LPIPS every N iterations (1=every iter)
        ...
        self.perc_loss_every_n_iter = perc_loss_every_n_iter
        self._cached_perc_loss = None
    ...
    def compute_loss(self, output: dotdict, batch: dotdict, loss: torch.Tensor, scalar_stats: dotdict, image_stats: dotdict):
        ...
        # Define the rgb_map keys to iterate over
        rgb_map_keys = ['rgb_map']
        if 'trans_env_rgb_direct' in output and batch.meta.iter < 99999999:
            rgb_map_keys.append('trans_env_rgb_direct') # rgb_map_keys here ['rgb_map', 'trans_env_rgb_direct']
            # pass
            
        prefix= ''  # Default prefix for loss names
        mask_to_use = (batch.msk > 0).float()  # Default mask to use
        for rgb_key in rgb_map_keys:
            if rgb_key in output:
                # Determine prefix for loss names
                prefix = '' if rgb_key == 'rgb_map' else 'trans_'

            # Perceptual loss (optionally computed every N iterations to save VGG forward pass)
            if self.perc_loss_weight > 0 and \
               ...
               batch.meta.n_rays[0].item() == -1:
                ...
                if batch.meta.iter % self.perc_loss_every_n_iter == 0:
                    _, perc_loss = compute_image_loss(output[rgb_key], batch.rgb, output.bg_color, mask_to_use, H, W, type=ImgLossType.PERC)
                    self._cached_perc_loss = perc_loss
                elif self._cached_perc_loss is not None:
                    perc_loss = self._cached_perc_loss.detach()
                else:
                    perc_loss = torch.zeros(1, device=output[rgb_key].device)
                scalar_stats[f'{prefix}perc_loss'] = perc_loss
                loss += self.perc_loss_weight * perc_loss

            # SSIM loss
            if self.ssim_loss_weight > 0 and \
               batch.meta.n_rays[0].item() == -1:
                if 'patch_h' in batch.meta and 'patch_w' in batch.meta:
                    H, W = batch.meta.patch_h[0].item(), batch.meta.patch_w[0].item()
                else:
                ...
                _, ssim_loss = compute_image_loss(output[rgb_key], batch.rgb, output.bg_color, mask_to_use, H, W, type=ImgLossType.SSIM, win_size=self.ssim_win_size)
                scalar_stats[f'{prefix}ssim_loss'] = ssim_loss
                loss += self.ssim_loss_weight * ssim_loss

            # MS-SSIM loss
            if self.msssim_loss_weight > 0 and \
               batch.meta.n_rays[0].item() == -1:
                if 'patch_h' in batch.meta and 'patch_w' in batch.meta:
                    H, W = batch.meta.patch_h[0].item(), batch.meta.patch_w[0].item()
                else:
                ...
                _, msssim_loss = compute_image_loss(output[rgb_key], batch.rgb, output.bg_color, mask_to_use, H, W, type=ImgLossType.MSSSIM, win_size=self.ssim_win_size)
                scalar_stats[f'{prefix}msssim_loss'] = msssim_loss
                loss += self.msssim_loss_weight * msssim_loss

            # Image reconstruction loss
            if self.img_loss_weight > 0:
                if 'img_loss_weight' in batch:
                    psnr, img_loss = compute_image_loss(output[rgb_key], batch.rgb, output.bg_color, mask_to_use, img_loss_weight=batch.img_loss_weight)
                else:
                    psnr, img_loss = compute_image_loss(output[rgb_key], batch.rgb, output.bg_color, mask_to_use)
                    scalar_stats[f'{prefix}psnr'] = psnr
                    scalar_stats[f'{prefix}img_loss'] = img_loss
                    
                    # Apply 0.1 weight for trans_env_rgb_direct loss
                    weight_multiplier = 1.0 if rgb_key == 'trans_env_rgb_direct' else 1.0
                    loss += self.img_loss_weight * weight_multiplier * img_loss
