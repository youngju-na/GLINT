...
class GlintSampler(Gaussian2DSampler):
    def __init__(self,
                 ...
                 env_max_gs: int = 5e5,  # control the maximum number of gaussians
                 ...
                 multi_view_patch_size: int = 2,  # patch size for NCC computation
                 multi_view_pixel_noise_th: float = 1.0,  # pixel noise threshold for geometric consistency
                 multi_view_sample_num: int = 51200,  # number of pixels to sample for multi-view loss
                 multi_view_every_n_iter: int = 5,  # compute multi-view loss every N iterations
                 wo_use_geo_occ_aware: bool = False, # not use the occlusion-aware weighting in geometric consistency

                 # PCD (interface gaussian) freeze parameters
                 pcd_freeze_enabled: bool = False,  # whether to freeze self.pcd after pcd_freeze_iter
                 pcd_freeze_iter: int = 15000,  # iteration after which self.pcd is frozen (no densify/prune/update)
        ...
        self.multi_view_every_n_iter = multi_view_every_n_iter
        self.wo_use_geo_occ_aware = wo_use_geo_occ_aware

        # PCD freeze parameters
        self.pcd_freeze_enabled = pcd_freeze_enabled
        self.pcd_freeze_iter = pcd_freeze_iter
        self.pcd_frozen = False  # state tracking for one-time freeze
        ...
        self.trans_env_bg_color = make_buffer(torch.Tensor([self.env_bg_brightness] * self.env_bg_channel))
    ...
    def update_dif_gaussians(self, batch: dotdict):
        if not self.training or torch.is_inference_mode_enabled(): return

        # Check if PCD should be frozen
        if self.pcd_freeze_enabled and batch.meta.iter >= self.pcd_freeze_iter:
            if not self.pcd_frozen:
                log(yellow_slim(f'[PCD FREEZE] Freezing self.pcd at iteration {batch.meta.iter}'))
                freeze_module(self.pcd)
                self.pcd_frozen = True
            # Still log num_pts but skip all updates
            ...
            return  # Skip all densification, pruning, and learning rate updates
    ...
    def store_dif_gaussian_output(self, middle: dotdict, batch: dotdict):
        ...
        if self.render_reflection and 'trans_map' in middle:
    ...
    def forward(self, batch: dotdict):
        ...
        if self.training and not self.trans_env_initialized and batch.meta.iter >= self.render_transmission_start_iter:
            ...
            self.init_trans_env(cfg.runner.optimizer, init_from=None)
        ...
        if self.use_base_tracing:
            if self.tracing_backend == 'cpp':
                # Validation checks before calling cpp backend
        ...
        valid_mask = (output.acc_map > 0.9).detach()  # threshold can be adjusted
        ...
        if batch.meta.iter >= self.render_transmission_start_iter:
            ...
            if self.tracing_backend == 'cpp':
                env_output_trans = self.diffop_trans.render_gaussians(
                    ...
                    self.trans_env_bg_color,  # Changed from self.env_bg_color to self.bg_color to match pcd
                    # self.env,
                    # self.pipe_env,
                    # self.env_bg_color,
                    0,
                ...
                if cfg.model_cfg.supervisor_cfg.plausibility_loss_weight > 0 or cfg.model_cfg.supervisor_cfg.trans_guidance_loss_weight > 0:
                    ...
                    trans_direct_output = self.render_gaussians(
                        viewpoint_camera, self.trans_env, self.pipe_trans_env, self.trans_env_bg_color, self.scale_mod
                    )
                    ...
                    if cfg.model_cfg.supervisor_cfg.plausibility_loss_weight > 0:
                        ...
                        output.trans_surf_norm_map = trans_direct_output.surf_norm_map
        ...
        if batch.meta.iter >= self.render_reflection_start_iter:
            ...
            if self.tracing_backend == 'cpp':
                ...
                if trans_ref_o is not None:
                    num_primary_rays = ref_o.shape[1]  # number of primary reflection rays (P)
            ...
            if self.training:
                ...
                self.last_output_env_refl = None  # not needed during inference
        
        
        # Prepare output for supervision and visualization
        output = self.store_env_gaussian_output(env_output_refl, env_output_trans, env_output_trans_refl, output, batch)
        
        # Compute multi-view consistency if enabled and a valid neighbor is available
        # Only compute every N iterations to save homography + NCC overhead
        if (self.training and
            batch.meta.iter >= cfg.model_cfg.supervisor_cfg.multi_view_start_iter and
            batch.meta.iter % self.multi_view_every_n_iter == 0 and
            self.get_multi_view_neighbor(batch) is not None):
        ...
        # Transmission map guidance masks computation (training only - requires GT data)
        if (self.training and
            'trans_dpt_map' in output and
            ...
            if cfg.model_cfg.supervisor_cfg.depth_discrepancy_threshold is not None:
                depth_identical_mask = output.trans_dpt_map <= (output.pcd_dpt_map + cfg.model_cfg.supervisor_cfg.depth_discrepancy_threshold)
                depth_larger_mask = output.trans_dpt_map > (output.pcd_dpt_map + cfg.model_cfg.supervisor_cfg.depth_discrepancy_threshold)
            else:
            ...
            if cfg.dataloader_cfg.dataset_cfg.use_diffren:
                ...
                if cfg.model_cfg.supervisor_cfg.use_intrinsics_guided_masks:
                    ...
                    intrinsics_guided_trans_mask = torch.ones_like(low_albedo_mask, dtype=torch.bool)
                    intrinsics_guided_opaque_mask = torch.zeros_like(low_albedo_mask, dtype=torch.bool)
    ...
    def get_multi_view_neighbor(self, batch: dotdict):
        candidates = []
        if hasattr(batch, 'meta_prev') and hasattr(batch, 'rgb_prev'):
            candidates.append((batch.meta_prev, batch.rgb_prev))
        if hasattr(batch, 'meta_next') and hasattr(batch, 'rgb_next'):
            candidates.append((batch.meta_next, batch.rgb_next))

        for meta, rgb in candidates:
            same_view = hasattr(meta, 'view_index') and hasattr(batch, 'view_index') and meta.view_index == batch.view_index
            same_latent = hasattr(meta, 'latent_index') and hasattr(batch, 'latent_index') and meta.latent_index == batch.latent_index
            if same_view and same_latent:
                continue
            return meta, rgb
        return None
    ...
    def compute_multi_view_consistency(self, batch: dotdict, output: dotdict):
        ...
        # multi_view_target = "trans_dpt_map" or "dpt_map"

        ref_dpt_map = rearrange(output.dpt_map, '1 (h w) 1 -> 1 h w', h=H, w=W)
        neighbor = self.get_multi_view_neighbor(batch)
        if neighbor is None:
            zero = torch.tensor(0.0, device=output.dpt_map.device)
            empty_mask = torch.zeros((1, H * W, 1), dtype=torch.bool, device=output.dpt_map.device)
            output.geo_loss = zero
            output.ncc_loss = zero
            return {
                'geo_consistency_mask': empty_mask,
                'geo_loss': output.geo_loss,
                'ncc_loss': output.ncc_loss,
            }

        nearest_meta, nearest_rgb = neighbor
        nearest_meta.rgb = nearest_rgb
        nearest_cam = to_x(prepare_gaussian_camera(nearest_meta, iterations=batch.meta.iter), torch.float)

        # Render interface Gaussian (self.pcd) from neighbor view for consistency check
        self.pipe_env.convert_SHs_python = True
        output_nearest = self.render_gaussians(nearest_cam, self.pcd, self.pipe_env, self.bg_color, self.scale_mod, override_color=None)
        self.pipe_env.convert_SHs_python = False
        ...
        mask = torch.zeros(0, dtype=torch.bool, device=output.dpt_map.device)
        ncc_loss = torch.tensor(0.0, device=output.dpt_map.device)
        if d_mask.sum() > 0:
            ...
            ## compute Homography
            ref_local_n = rearrange(output.norm_map, '1 (h w) c -> h w c', h=H, w=W)
            ref_local_n = ref_local_n.reshape(-1, 3)[valid_indices]

            ref_local_d = ref_dpt_map.reshape(-1)[valid_indices]
