...
class Gaussian2DSampler(VolumetricVideoModule):
    def __init__(self,
                 ...
                 init_ior: float = 1.5,  # index of refraction initialization
                 init_transmission_coeff: float = 0.01,  # transmission coefficient initialization
        ...
        self.init_ior = init_ior
        self.init_transmission_coeff = init_transmission_coeff
        ...
        self.pcd = GaussianModel(
            ...
            init_ior=self.init_ior,
            init_transmission_coeff=self.init_transmission_coeff,
            max_gs=self.max_gs,
            max_gs_threshold=self.max_gs_threshold
        )
        
        # 1. Initialize a new GaussianModel instance (G_trans)
        self.trans_env = GaussianModel(
            xyz=xyz,
            colors=colors,
            init_occ=self.init_occ,
            init_scale=None,
        ...
        if self.use_optix_tracing:
            ...
            self.diffop_trans = HardwareRendering()  # for transmission rendering
    ...
    def extract_input(self, batch: dotdict, n_rays: int = -1, patch_size: List[int] = [-1, -1]):
        ...
        if could_skip_rays:
            ...
            # Otherwise, recompute the near, far, time since we actually skip the `VolumetricVideoModule`
    ...
    def store_gaussian_output(self, output: dotdict, batch: dotdict):
        ...
        if self.render_reflection and 'transparency' in output:
            output.trans_map = output.transparency[None].permute(0, 2, 3, 1).reshape(1, -1, output.transparency.shape[0])  # (B, H * W, 1)
        if self.render_reflection and 'ior' in output:
            output.ior_map = output.ior[None].permute(0, 2, 3, 1).reshape(1, -1, output.ior.shape[0])  # (B, H * W, 1)
        if self.render_reflection and 'roughness' in output:
            output.roughness_map = output.roughness[None].permute(0, 2, 3, 1).reshape(1, -1, output.roughness.shape[0])
