...
class VolumetricVideoModel(nn.Module):
    ...
    def prepare_diffren_params(self, batch: dotdict):
        # Decode RGB-encoded float depth maps already in [0, 1] scale

        def decode_normalized_rgb_depth(depth_rgb: torch.Tensor) -> torch.Tensor:
            """
            Decode normalized RGB float depth maps in [0, 1] range.
            Each channel already represents R, G, B / 255.

            Args:
                depth_rgb (torch.Tensor): shape [B, N, 3], float32 in [0, 1]

            Returns:
                torch.Tensor: Decoded float depth, shape [B, N]
            """
            R = depth_rgb[..., 0]
            G = depth_rgb[..., 1]
            B = depth_rgb[..., 2]

            # Assume RGB already ∈ [0, 1], so simulate byte reconstruction:
            depth = (R * 1.0 + G * (1.0 / 256.0) + B * (1.0 / (256.0**2)))
        ...
        if 'depth' in batch:
            if batch.depth.ndim == 3 and batch.depth.shape[-1] == 3:
                batch.depth = decode_normalized_rgb_depth(batch.depth)[..., None] # shape: [B, N, 1]
    ...
    def forward(self, batch: dotdict):
        ...
        batch = self.prepare_diffren_params(batch)
