...
class SmoothedValue(object):
    ...
    def update(self, value):
        ...
        elif hasattr(value, '__array__'): value = float(value.mean())  # numpy arrays
...
class TensorboardRecorder:
    ...
    def update_scalar_stats(self, scalar_stats: dotdict):
        ...
        for k, v in scalar_stats.items():
            if isinstance(self.scalar_stats[k], SmoothedValue) and (
                isinstance(v, float) or
                (isinstance(v, torch.Tensor) and (v.is_floating_point() or v.requires_grad)) or
                (hasattr(v, '__array__') and getattr(v, 'dtype', None) is not None and getattr(v.dtype, 'kind', None) == 'f')
            ):
