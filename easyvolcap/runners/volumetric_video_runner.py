...
from easyvolcap.utils.data_utils import add_iter, to_cuda, init_diffren_params
...
class VolumetricVideoRunner:  # a plain and simple object controlling the training loop
    def __init__(self,
                 ...
                 debug_mode: bool = False,  # if true, will not perform any training, just load the model and run the test
                 ...
                 ignore_eval_error: bool = False,  # errors in evaluation will not affect training
                 record_images_to_tb: bool = False,  # when testing, save images to tensorboard (slow, disable by default)
                 ...
                 test_using_inference_mode: bool = False,  # inference_mode can break custom autograd/TorchScript renderers during validation
                 ...
                 timer_sync_cuda: bool = False,  # will explicitly call torch.cuda.synchronize() before collecting
        ...
        self.eval_ep = eval_ep # report validation stats
        self.save_ep = save_ep
        self.debug_mode = debug_mode  # if true, will not perform any training, just load the model and run the test
        
        if self.debug_mode:
            self.eval_ep = 1  # only one epoch for debugging
    ...
    def train_generator(self, begin_epoch: int, yield_every: int = 1):
        ...
        while flying_batch.meta.iter >= 0:  # control number of iterations explicitly
            ...
            if (iter + 1) % self.log_interval == 0 and not get_rank():
                ...
                # For recording onto the tensorboard (SmoothedValue.update handles tensor->scalar conversion)
    ...
    def test_generator(self, epoch: int, yield_every: int = 1):
        ...
        torch.cuda.synchronize()  # wait for async visualization copies to finish
