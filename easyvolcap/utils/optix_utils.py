...
class HardwareRendering(nn.Module):
    ...
    def render_gaussians(self,
        ...
        v, f = self.build_bvh(pcd, batch, rebuild=self.training or not rebuild)
        ...
        if pcd.render_reflection:
            ...
            # For reflection rendering, we need to render specular and (roughness, no use for now)
                ...
                others_precomp = torch.cat([pcd.get_specular, pcd.get_transmission_coeff], dim=-1).contiguous()  # (P, 2)
        ...
        if pcd.render_reflection:
            output.update(dotdict(
                ...
                transparency=aux[..., pcd.specular_channels:pcd.specular_channels+1].permute(2, 0, 1))  # (1, H, W)
            )
