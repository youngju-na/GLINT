...
class Visualization(Enum):
    ...
    SHADING = auto()
    ROUGHNESS = auto()
    TRANSPARENCY = auto()  # transparency map, not the same as alpha
    SPECULAR = auto()
    DIFFUSE = auto()
    REFLECTION = auto()
    TRANS_REFLECTION = auto()
    TRANSMISSION = auto()  # transmission map, which is a transmitted ray color
    TRANS_DEPTH = auto()  # transmission depth, which is a depth of the transmitted ray
    TRANS_NORMAL = auto()  # transmission normal, which is a normal of the transmitted ray
    OPAQUE = auto()  # opaque map, which is a mask of the opaque regions
    TRANS_GUIDANCE_ANGLE = auto()  # angle attenuation weight used by transmission guidance
    TRANS_GUIDANCE_DEPTH = auto()  # depth attenuation weight used by transmission guidance
    TRANS_GUIDANCE_WEIGHT = auto()  # combined reliability weight used by transmission guidance
    
    # rasterize env, trans_env
    ENV_RENDER = auto()  # rasterized environment map
    TRANS_ENV_RENDER = auto()  # rasterized transmission environment map
...
def init_diffren_params(batch):
    # select diffren from batch
    # currently the batch.depth is 3-channel, need to consider this is depth.
    
    def decode_rgb_to_depth_batched_cuda(depth_rgb: torch.Tensor) -> torch.Tensor:
        """
        Decode a batch of RGB-encoded depth maps into float depth (range [0, 1]).

        Args:
            depth_rgb (torch.Tensor): shape [B, N, 3], dtype uint8 or float32, device CUDA or CPU

        Returns:
            torch.Tensor: Decoded float depth, shape [B, N], dtype float32
        """
        # Ensure float32
        if depth_rgb.dtype != torch.float32:
            depth_rgb = depth_rgb.float()
        ...
        # Compute depth
        depth = (R + G * 256 + B * 256**2) / (256**3 - 1)
        return depth  # shape: [B, N], float32

    if 'depth' in batch:
        if batch.depth.ndim == 3 and batch.depth.shape[-1] == 3:
            batch.depth = decode_rgb_to_depth_batched_cuda(batch.depth)[..., None]
