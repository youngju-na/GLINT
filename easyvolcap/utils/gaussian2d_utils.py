...
def get_rays(output, scale=1.0):
        W, H = int(output.image_width/scale), int(output.image_height/scale)
        ix, iy = torch.meshgrid(
            torch.arange(W), torch.arange(H), indexing='xy')
        rays_d = torch.stack(
                    [(ix-output.Cx/scale) / output.Fx * scale,
                    (iy-output.Cy/scale) / output.Fy * scale,
                    torch.ones_like(ix)], -1).float().cuda()
        return rays_d

def get_k(output, scale=1.0):
    K = torch.eye(3, dtype=torch.float32, device=output.K.device)
    K[0, 0] = output.Fx / scale
    K[1, 1] = output.Fy / scale
    K[0, 2] = output.Cx / scale
    K[1, 2] = output.Cy / scale
    return K

def get_inv_k(output, scale=1.0):
    K = torch.eye(3, dtype=torch.float32, device=output.K.device)
    K[0, 0] = output.Fx / scale
    K[1, 1] = output.Fy / scale
    K[0, 2] = output.Cx / scale
...
def prepare_gaussian_camera(batch, iterations=0):
    output = dotdict()
    ...
    output.image_name = f"{int(iterations):04d}_{int(batch.camera_index):04d}"

    if 'rgb' in batch:
        output.image = batch.rgb[0].reshape(cpu_H, cpu_W, 3)  # use cpu image
    else:
        output.image = torch.zeros(cpu_H, cpu_W, 3)  # dummy for inference without GT
    output.gray_image = 0.2989 * output.image[..., 0:1] + 0.5870 * output.image[..., 1:2] + 0.1140 * output.image[..., 2:3]
    
    output.ncc_scale = 1.0
    
    output.K = K
    output.R = R
    output.T = T
    output.Cx = cpu_K[0, 2]  # use cpu K
    output.Cy = cpu_K[1, 2]  # use cpu K
    output.Fx = cpu_K[0, 0]  # use cpu K
    output.Fy = cpu_K[1, 1]  # use cpu K
    
    output.R_c2w = cpu_R.mT
    # output.T_c2w = (-cpu_R.mT @ cpu_T)[..., 0]
    ...
    output.get_k = lambda scale=1.0: get_k(output, scale)
    output.get_inv_k = lambda scale=1.0: get_inv_k(output, scale)
    output.get_rays = lambda scale=1.0: get_rays(output, scale)
    # return rgb and gray image
    output.get_image = lambda: (output.image, output.gray_image)
...
def prepare_gaussian_neighboring_camera(batch):
    pass
...
def inverse_ior_activation(n):
    # Previous: return torch.log(torch.clamp(n - 1.0, 1e-6, 1e6))
...
class GaussianModel(nn.Module):
    ...
    def reset_transmission_coeff(self, reset_transmission_coeff: float = 0.01, optimizer: Optimizer = None, prefix: str = ''):
        ...
        # Gaussians that originally had low transmission_coeff (opaque) remain opaque even after reset
    ...
    def replace_tensor_to_optimizer(self, tensor: torch.Tensor, name: str, optimizer: Optimizer = None, prefix: str = ''):
        if getattr(tensor, 'is_inference', lambda: False)():
            tensor = tensor.clone()
    ...
    def cat_tensors_to_optimizer(self, tensors_dict: dotdict, optimizer: Optimizer, prefix: str = ''):
        ...
        for group in optimizer.param_groups:
            ...
            if getattr(extension_tensor, 'is_inference', lambda: False)():
                extension_tensor = extension_tensor.clone()
            stored_state = optimizer.state.get(group['params'][0], None)
            if stored_state is not None:
    ...
    def get_points_from_depth(self, fov_camera, depth, scale=1):
        ...
        R = torch.tensor(fov_camera.R_c2w).float().cuda()
        T = torch.tensor(fov_camera.T.squeeze()).float().cuda()
...
def render(
    ...
    trans_mask: torch.Tensor = None, # ! New argument
    device: str = 'cuda'
):
    ...
    if pc.render_reflection and pc.specular_channels == 1: from diff_surfel_rasterization_wet_ch06 import GaussianRasterizationSettings, GaussianRasterizer
    ...
    elif pc.render_reflection and pc.specular_channels == 3: from diff_surfel_rasterization_wet_ch08 import GaussianRasterizationSettings, GaussianRasterizer
    ...
    # Filter gaussians based on trans_mask if provided
    if trans_mask is not None:
        # Ensure trans_mask is a boolean tensor
        trans_mask = trans_mask.bool().flatten()
        
        means3D = means3D[trans_mask]
        means2D = means2D[trans_mask]
        opacity = opacity[trans_mask]
        
        if scales is not None:
            scales = scales[trans_mask]
        if rotations is not None:
            rotations = rotations[trans_mask]
        if cov3D_precomp is not None:
            cov3D_precomp = cov3D_precomp[trans_mask]
        if shs is not None:
            shs = shs[trans_mask]
        if colors_precomp is not None:
            colors_precomp = colors_precomp[trans_mask]
    ...
    if pc.render_reflection and colors_precomp is not None:
        ...
        transmission_coeff = pc.get_transmission_coeff
        if trans_mask is not None:
            # Ensure trans_mask is a boolean tensor
            trans_mask = trans_mask.bool().flatten()
            ior = ior[trans_mask]
            # roughness = roughness[trans_mask]
            transmission_coeff = transmission_coeff[trans_mask]
        # Pass ior, roughness, and transmission coefficient to the rasterizer
        colors_precomp = torch.cat([colors_precomp, ior, specular, transmission_coeff], dim=-1)
    ...
    if pc.render_reflection:
        # The rasterizer now returns a packed tensor. Unpack it.
        # The order is assumed to be: ior, roughness, transmission_coeff
        output.update(dotdict(
            ior=rendered_image[3:3+pc.specular_channels],  # (1, H, W)
            specular=rendered_image[3+pc.specular_channels:3+pc.specular_channels+1],  # (1, H, W)
            transparency=rendered_image[3+pc.specular_channels+1:3+pc.specular_channels+2]  # (1, H, W)
        ))

    # Those Gaussians that were frustum culled or had a radius of 0 were not visible.
    # They will be excluded from value updates used in the splitting criteria.
    full_visibility_filter = torch.zeros(pc.get_xyz.shape[0], dtype=torch.bool, device=device)
    if trans_mask is not None:
        full_visibility_filter[trans_mask] = radii > 0
    else:
        full_visibility_filter = radii > 0
    
    full_radii = torch.zeros(pc.get_xyz.shape[0], device=device)
    if trans_mask is not None:
        full_radii[trans_mask] = radii.float()
    else:
        full_radii = radii.float()

    full_weight_accumulate = torch.zeros(pc.get_xyz.shape[0], 1, device=device)
    if trans_mask is not None:
        full_weight_accumulate[trans_mask] = weight.detach().clone()
    else:
        full_weight_accumulate = weight.detach().clone()

    output.update(dotdict(
        viewspace_points=means2D,  # (P, 3)
        visibility_filter=full_visibility_filter,  # (P,)
        radii=full_radii,  # (P,)
        weight_accumulate=full_weight_accumulate  # (P,)
    ))
