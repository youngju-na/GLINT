...
def load_points(points_file: str = None):
    ...
    if exists(points_file):
        ...
        xyz, rgbs = load_sfm_ply(points_file)
    else:
        try:
            print(f'Loading points from {points_file.replace(".ply", ".bin")}')
            xyz, rgbs, _ = read_points3D_binary_custom(points_file.replace(".ply", ".bin"))
        except:
            print(f'Loading points from {points_file.replace(".ply", ".txt")}')
            xyz, rgbs, _ = read_points3D_text_custom(points_file.replace(".ply", ".txt"))
            
    # save as points3D.ply
    if xyz is not None:
        print(f'Saving points to {points_file}')
        save_sfm_ply(points_file, xyz, rgbs)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--points_file', type=str, default='sparse/points3D.ply')
