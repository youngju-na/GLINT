...
class VolumetricVideoDataset(Dataset):
    def __init__(self,
                 ...
                 use_diffren: bool = False,
                 diffren_params: list = ["normal", "depth", "diffuse_albedo", "basecolor", "roughness", "metallic"],
                 
                 use_neighbors: bool = False,  # use neighbors for DiffusionRenderer priors
                 neighbor_params: list = ["normal", "depth", "diffuse_albedo"],  # DiffusionRenderer parameters to use for neighbors
                 neighbor_window: int = 5,  # temporal window ±neighbor_window
                 
                 sky_mask_dir: str = 'sky_masks',  # directory for sky masks
                 use_sky_masks: bool = False,  # whether to use sky masks
        ...
        self.diffren_dir = diffren_dir  # for those methods who use DiffusionRenderer priors
        self.sky_masks_dir = sky_mask_dir  # directory for sky masks
        self.use_sky_masks = use_sky_masks  # whether to use sky masks
        ...
        self.neighbor_params = neighbor_params  # DiffusionRenderer parameters to use for neighbors
    ...
    def load_paths(self):
        ...
        if self.use_sky_masks:
            self.sky_masks = np.asarray([im.replace(self.images_dir, self.sky_masks_dir) for im in self.ims.ravel()]).reshape(self.ims.shape)
            if not exists(self.sky_masks[0, 0]):
                self.sky_masks = np.asarray([mk.replace('.png', '.jpg') for mk in self.sky_masks.ravel()]).reshape(self.sky_masks.shape)
            if not exists(self.sky_masks[0, 0]):
                self.sky_masks = np.asarray([mk.replace('.jpg', '.png') for mk in self.sky_masks.ravel()]).reshape(self.sky_masks.shape)
            if not exists(self.sky_masks[0, 0]):  # Two types of commonly used sky mask directories
                self.sky_masks = np.asarray([mk.replace(self.sky_masks_dir, 'sky_masks') for mk in self.sky_masks.ravel()]).reshape(self.sky_masks.shape)
            if not exists(self.sky_masks[0, 0]):
                self.sky_masks = np.asarray([mk.replace('sky_masks', 'sky_mask') for mk in self.sky_masks.ravel()]).reshape(self.sky_masks.shape)
            if not exists(self.sky_masks[0, 0]):
                self.sky_masks = np.asarray([mk.replace('sky_masks', 'sky_msk') for mk in self.sky_masks.ravel()]).reshape(self.sky_masks.shape)
            self.sky_mks_dir = join(*split(dirname(self.sky_masks[0, 0]))[:-1])
        ...
        if self.use_diffren:
            self.diffrens = dotdict()
            for param in self.diffren_params:
                param_dir = join(self.diffren_dir, param) # ./diffren_priors/normals, ./diffren_priors/depths, etc.
                
                self.diffrens[param] = np.asarray([im.replace(self.images_dir, param_dir) for im in self.ims.ravel()]).reshape(self.ims.shape)
                if not exists(self.diffrens[param][0, 0]):
                    # get png instead of jpg
                    self.diffrens[param] = np.asarray([dp.replace('.jpg', '.png') for dp in self.diffrens[param].ravel()]).reshape(self.diffrens[param].shape)
                    
        # neighbor diffusion-renderer prior to patchmatching verification
        if self.use_neighbors:
            self.neighbors = dotdict()
            for param in self.neighbor_params:
                param_dir = join(self.diffren_dir, 'neighbors', param)
                
                self.neighbors[param] = np.asarray([im.replace(self.images_dir, param_dir) for im in self.ims.ravel()]).reshape(self.ims.shape)
                if not exists(self.neighbors[param][0, 0]):
                    # get png instead of jpg
                    self.neighbors[param] = np.asarray([dp.replace('.jpg', '.png') for dp in self.neighbors[param].ravel()]).reshape(self.neighbors[param].shape)   
    ...
    def load_bytes(self):
        ...
        if self.use_sky_masks:
            self.sky_masks_bytes, self.Ks, self.Hs, self.Ws = \
                load_resize_undist_ims_bytes(self.sky_masks, ori_Ks.numpy(), ori_Ds.numpy(), ratio, self.center_crop_size,
                                             f'Loading sky mask bytes for {blue(self.sky_masks_dir)} {magenta(self.split.name)}',
                                             decode_flag=cv2.IMREAD_GRAYSCALE, dist_opt_K=self.dist_opt_K, backend=self.backend, encode_ext=self.encode_ext, num_workers=self.dataloading_workers)
        ...
        if self.use_diffren: #! check if DiffusionRenderer priors are available
            self.diffrens_bytes = dotdict()
            for param in self.diffren_params:
                self.diffrens_bytes[param], self.Ks, self.Hs, self.Ws = \
                    load_resize_undist_ims_bytes(self.diffrens[param], ori_Ks.numpy(), ori_Ds.numpy(), ratio, self.center_crop_size,
                                                 f'Loading {param} bytes for {blue(self.diffren_dir)} {magenta(self.split.name)}',
                                                 dist_opt_K=self.dist_opt_K, backend=self.backend, encode_ext=self.encode_ext, num_workers=self.dataloading_workers)
        ...
        if self.immask_crop:  # a little bit wasteful but acceptable for now
            ...
            if hasattr(self, 'diffrens_bytes'): #! DiffusionRenderer prior cropping
                for param in self.diffren_params:
                    self.diffrens_bytes[param], _, _, _, _, _, _ = \
                        decode_crop_fill_ims_bytes(self.diffrens_bytes[param], self.mks_bytes, self.Ks.numpy(), self.Rs.numpy(), self.Ts.numpy(), bounds.numpy(), f'Cropping msks {param} for {blue(self.data_root)} {magenta(self.split.name)}', encode_ext=self.encode_ext, num_workers=self.dataloading_workers)
        ...
        # Only fill the background regions
            ...
            if hasattr(self, 'diffrens_bytes'): #! DiffusionRenderer prior filling
                for param in self.diffren_params:
                    self.diffrens_bytes[param] = decode_fill_ims_bytes(self.diffrens_bytes[param], self.mks_bytes, f'Filling {param} imgs for {blue(self.data_root)} {magenta(self.split.name)}', encode_ext=self.encode_ext, num_workers=self.dataloading_workers)
        ...
        if self.cache_raw:
            ...
            if hasattr(self, 'diffrens_bytes'):
                self.diffrens_bytes = dotdict({
                    param: to_tensor([load_image_from_bytes(x, normalize=False) for x in tqdm(self.diffrens_bytes[param], desc=f'Caching {param} for {blue(self.data_root)} {magenta(self.split.name)}')])
                    for param in self.diffren_params
                })
            ...
            if hasattr(self, 'diffrens_bytes'):
                self.diffrens_bytes = dotdict({
                    param: UnstructuredTensors(self.diffrens_bytes[param])
                    for param in self.diffren_params
                })
    ...
    def get_image_bytes(self, view_index: int, latent_index: int):
        ...
        if self.use_diffren: #! check if DiffusionRenderer priors are available
            diffrens_bytes = dotdict()
            for param in self.diffren_params:
                diffrens_bytes[param] = self.diffrens_bytes[param][view_index * self.n_latents + latent_index]
        else:
            diffrens_bytes = None
            
        if self.use_sky_masks:
            sky_masks_bytes = self.sky_masks_bytes[view_index * self.n_latents + latent_index]
        else:
            sky_masks_bytes = None
        
        return im_bytes, mk_bytes, wt_bytes, dp_bytes, bg_bytes, nm_bytes, diffrens_bytes, sky_masks_bytes

    def get_image_from_bytes(self, view_index: int, latent_index: int):
        # Load bytes (rgb, msk, wet, bg)
        im_bytes, mk_bytes, wt_bytes, dp_bytes, bg_bytes, nm_bytes, diffren_bytes, sky_bytes = self.get_image_bytes(view_index, latent_index)
        rgb, msk, wet, dpt, bkg, norm, diffrens, sky_masks, neighbors = None, None, None, None, None, None, None, None, None
        ...
        # Load DiffusionRenderer priors from bytes
        if diffren_bytes is not None:
            diffrens = dotdict()
            for param in self.diffren_params:
                if self.cache_raw:
                    diffrens[param] = torch.as_tensor(diffren_bytes[param])
                else:
                    diffrens[param] = torch.as_tensor(load_image_from_bytes(diffren_bytes[param], normalize=True))
                    
        # Load sky masks from bytes
        if sky_bytes is not None:
            if self.cache_raw:
                sky_masks = torch.as_tensor(sky_bytes)
            else:
                sky_masks = torch.as_tensor(load_image_from_bytes(sky_bytes, normalize=True)[..., :1])

        return rgb, msk, wet, dpt, bkg, norm, diffrens, sky_masks
    ...
    def get_image_from_disk(self, view_index: int, latent_index: int, output: dotdict = None):
        ...
        # Maybe load DiffusionRenderer priors here
        if self.use_diffren:
            diffrens = dotdict()
            for param in self.diffren_params:
                diffrens[param] = self.diffrens[param][view_index][latent_index]
                diffrens[param], _, _, _ = load_resize_undist_im_bytes(diffrens[param], K.numpy(), D.numpy(), ratio, self.center_crop_size, dist_opt_K=self.dist_opt_K, backend=self.backend, encode=False)
        ...
        if self.immask_crop:  # a little bit wasteful but acceptable for now
            ...
            if 'diffrens' in locals():
                for param in self.diffren_params:
                    diffrens[param], _, _, _, _, _, _ = decode_crop_fill_im_bytes(diffrens[param], mk, K, R.numpy(), T.numpy(), bounds, decode=False, encode=False)
            
            if 'sky_masks' in locals():
                sky_masks, _, _, _, _, _, _ = decode_crop_fill_im_bytes(sky_masks, mk, K, R.numpy(), T.numpy(), bounds, decode=False, encode=False)
            ...
            if 'diffrens' in locals():
                for param in self.diffren_params:
                    diffrens[param], _, _, _, _, _, _ = decode_crop_fill_im_bytes(diffrens[param], np.ones_like(im[..., :1]), K, R.numpy(), T.numpy(), bounds, decode=False, encode=False)
            
            if 'sky_masks' in locals():
                sky_masks, _, _, _, _, _, _ = decode_crop_fill_im_bytes(sky_masks, np.ones_like(im[..., :1]), K, R.numpy(), T.numpy(), bounds, decode=False, encode=False)
            ...
            if 'diffrens' in locals():
                for param in self.diffren_params:
                    diffrens[param] = (diffrens[param] * (mk / 255)).clip(0, 255)
            if 'sky_masks' in locals():
                sky_masks = (sky_masks * (mk / 255)).clip(0, 255)
        ...
        if 'diffrens' not in locals(): diffrens = None
        else:
            for param in self.diffren_params:
                diffrens[param] = torch.as_tensor(normalize_image(diffrens[param]))
        
        if 'sky_masks' not in locals(): sky_masks = None
        else: sky_masks = torch.as_tensor(normalize_image(sky_masks))
    
        return im, mk, wt, dp, bg, nm, diffrens, sky_masks
    ...
    def get_ground_truth(self, index):
        ...
        output_neighbors_prev, output_neighbors_next = None, None
        if 'TRAIN' in self.split.name and self.use_neighbors:
            # Calculate neighbor indices with bounds checking
            prev_index = max(0, index - self.neighbor_window)
            next_index = min(len(self) - 1, index + self.neighbor_window)
            output_neighbors_prev = self.get_metadata(prev_index)  
            output_neighbors_next = self.get_metadata(next_index)
            output.meta_prev = output_neighbors_prev
            output.meta_next = output_neighbors_next
        
        rgb, msk, wet, dpt, bkg, norm, diffrens, sky_masks = self.get_image(output.view_index, output.latent_index, output)  # H, W, 3
        
        rgb_prev, msk_prev, wet_prev, dpt_prev, bkg_prev, norm_prev, diffrens_prev, sky_masks_prev = None, None, None, None, None, None, None, None
        rgb_next, msk_next, wet_next, dpt_next, bkg_next, norm_next, diffrens_next, sky_masks_next = None, None, None, None, None, None, None, None
        
        if output_neighbors_prev is not None:
            rgb_prev, msk_prev, wet_prev, dpt_prev, bkg_prev, norm_prev, diffrens_prev, sky_masks_prev = self.get_image(output_neighbors_prev.view_index,
                                                                                                                                    output_neighbors_prev.latent_index,
                                                                                                                                    output_neighbors_prev)
        if output_neighbors_next is not None:
            rgb_next, msk_next, wet_next, dpt_next, bkg_next, norm_next, diffrens_next, sky_masks_next = self.get_image(output_neighbors_next.view_index,
                                                                                                                                          output_neighbors_next.latent_index,
                                                                                                                                          output_neighbors_next)
        H, W = rgb.shape[:2]
        ...
        if not self.disk_dataset:
            if self.immask_crop:  # these variables are only available when loading gts
                ...
                meta_prev = dotdict()
                meta_prev.crop_x = self.crop_xs[output.meta_prev.view_index, self.virtual_to_physical(output.meta_prev.latent_index)]
                meta_prev.crop_y = self.crop_ys[output.meta_prev.view_index, self.virtual_to_physical(output.meta_prev.latent_index)]
                meta_prev.orig_h = self.orig_hs[output.meta_prev.view_index, self.virtual_to_physical(output.meta_prev.latent_index)]
                meta_prev.orig_w = self.orig_ws[output.meta_prev.view_index, self.virtual_to_physical(output.meta_prev.latent_index)]
                output.meta_prev.update(meta_prev)

                meta_next = dotdict()
                meta_next.crop_x = self.crop_xs[output.meta_next.view_index, self.virtual_to_physical(output.meta_next.latent_index)]
                meta_next.crop_y = self.crop_ys[output.meta_next.view_index, self.virtual_to_physical(output.meta_next.latent_index)]
                meta_next.orig_h = self.orig_hs[output.meta_next.view_index, self.virtual_to_physical(output.meta_next.latent_index)]
                meta_next.orig_w = self.orig_ws[output.meta_next.view_index, self.virtual_to_physical(output.meta_next.latent_index)]
                output.meta_next.update(meta_next)
                ...
                if diffrens is not None:
                    for param in self.diffren_params:
                        diffrens[param] = diffrens[param][y:y + h, x:x + w]
                if sky_masks is not None:
                    sky_masks = sky_masks[y:y + h, x:x + w]
                
                # Apply same cropping to neighbor images
                if rgb_prev is not None:
                    rgb_prev = rgb_prev[y:y + h, x:x + w]
                    msk_prev = msk_prev[y:y + h, x:x + w]
                    wet_prev = wet_prev[y:y + h, x:x + w]
                    if dpt_prev is not None: dpt_prev = dpt_prev[y:y + h, x:x + w]
                    if bkg_prev is not None: bkg_prev = bkg_prev[y:y + h, x:x + w]
                    if norm_prev is not None: norm_prev = norm_prev[y:y + h, x:x + w]
                    if diffrens_prev is not None:
                        for param in self.diffren_params:
                            diffrens_prev[param] = diffrens_prev[param][y:y + h, x:x + w]
                    if sky_masks_prev is not None:
                        sky_masks_prev = sky_masks_prev[y:y + h, x:x + w]
                
                if rgb_next is not None:
                    rgb_next = rgb_next[y:y + h, x:x + w]
                    msk_next = msk_next[y:y + h, x:x + w]
                    wet_next = wet_next[y:y + h, x:x + w]
                    if dpt_next is not None: dpt_next = dpt_next[y:y + h, x:x + w]
                    if bkg_next is not None: bkg_next = bkg_next[y:y + h, x:x + w]
                    if norm_next is not None: norm_next = norm_next[y:y + h, x:x + w]
                    if diffrens_next is not None:
                        for param in self.diffren_params:
                            diffrens_next[param] = diffrens_next[param][y:y + h, x:x + w]
                    if sky_masks_next is not None:
                        sky_masks_next = sky_masks_next[y:y + h, x:x + w]
        ...
        if (len(render_ratio.shape) and  # avoid length of 0-d tensor error, check length of shape
            ...
            if diffrens is not None:
                for param in self.diffren_params:
                    diffrens[param] = as_torch_func(partial(cv2.resize, dsize=(W, H), interpolation=cv2.INTER_AREA))(diffrens[param])
            if sky_masks is not None:
                sky_masks = as_torch_func(partial(cv2.resize, dsize=(W, H), interpolation=cv2.INTER_AREA))(sky_masks)

            # Apply same resizing to neighbor images
            if rgb_prev is not None:
                rgb_prev = as_torch_func(partial(cv2.resize, dsize=(W, H), interpolation=cv2.INTER_AREA))(rgb_prev)
                msk_prev = as_torch_func(partial(cv2.resize, dsize=(W, H), interpolation=cv2.INTER_AREA))(msk_prev)
                wet_prev = as_torch_func(partial(cv2.resize, dsize=(W, H), interpolation=cv2.INTER_AREA))(wet_prev)
                if dpt_prev is not None: dpt_prev = as_torch_func(partial(cv2.resize, dsize=(W, H), interpolation=cv2.INTER_AREA))(dpt_prev)
                if bkg_prev is not None: bkg_prev = as_torch_func(partial(cv2.resize, dsize=(W, H), interpolation=cv2.INTER_AREA))(bkg_prev)
                if norm_prev is not None: norm_prev = as_torch_func(partial(cv2.resize, dsize=(W, H), interpolation=cv2.INTER_AREA))(norm_prev)
                if diffrens_prev is not None:
                    for param in self.diffren_params:
                        diffrens_prev[param] = as_torch_func(partial(cv2.resize, dsize=(W, H), interpolation=cv2.INTER_AREA))(diffrens_prev[param])
                if sky_masks_prev is not None:
                    sky_masks_prev = as_torch_func(partial(cv2.resize, dsize=(W, H), interpolation=cv2.INTER_AREA))(sky_masks_prev)

            if rgb_next is not None:
                rgb_next = as_torch_func(partial(cv2.resize, dsize=(W, H), interpolation=cv2.INTER_AREA))(rgb_next)
                msk_next = as_torch_func(partial(cv2.resize, dsize=(W, H), interpolation=cv2.INTER_AREA))(msk_next)
                wet_next = as_torch_func(partial(cv2.resize, dsize=(W, H), interpolation=cv2.INTER_AREA))(wet_next)
                if dpt_next is not None: dpt_next = as_torch_func(partial(cv2.resize, dsize=(W, H), interpolation=cv2.INTER_AREA))(dpt_next)
                if bkg_next is not None: bkg_next = as_torch_func(partial(cv2.resize, dsize=(W, H), interpolation=cv2.INTER_AREA))(bkg_next)
                if norm_next is not None: norm_next = as_torch_func(partial(cv2.resize, dsize=(W, H), interpolation=cv2.INTER_AREA))(norm_next)
                if diffrens_next is not None:
                    for param in self.diffren_params:
                        diffrens_next[param] = as_torch_func(partial(cv2.resize, dsize=(W, H), interpolation=cv2.INTER_AREA))(diffrens_next[param])
                if sky_masks_next is not None:
                    sky_masks_next = as_torch_func(partial(cv2.resize, dsize=(W, H), interpolation=cv2.INTER_AREA))(sky_masks_next)
        ...
        if (len(render_center_crop_ratio.shape) and  # avoid length of 0-d tensor error, check length of shape
            ...
            # Apply same center crop to neighbor images
            if rgb_prev is not None:
                rgb_prev = rgb_prev[y: y + h, x: x + w, :]
                msk_prev = msk_prev[y: y + h, x: x + w, :]
                wet_prev = wet_prev[y: y + h, x: x + w, :]
            ...
            self.crop_ixts(output.meta_prev, x, y, w, h)
            self.crop_ixts(output.meta_next, x, y, w, h)
        ...
        if should_sample_patch:
            ...
            # Apply same patch sampling to neighbor images
            if rgb_prev is not None:
                rgb_prev = rgb_prev[y: y + h, x: x + w, :]
                msk_prev = msk_prev[y: y + h, x: x + w, :]
                wet_prev = wet_prev[y: y + h, x: x + w, :]
        ...
        if diffrens is not None:
            for param in self.diffren_params:
                if diffrens[param].ndim == 3 and diffrens[param].shape[-1] == 3:  # RGB or 3D vector
                    output[param] = diffrens[param].reshape(-1, 3)
                else:
                    output[param] = diffrens[param].reshape(-1, 1)
        if sky_masks is not None: output.sky_masks = sky_masks.reshape(-1, 1)  # full mask

        # Store neighbor data
        if rgb_prev is not None:
            output.rgb_prev = rgb_prev.reshape(-1, 3)
            output.msk_prev = msk_prev.reshape(-1, 1)
            output.wet_prev = wet_prev.reshape(-1, 1)
            if dpt_prev is not None: output.dpt_prev = dpt_prev.reshape(-1, 1)
            if bkg_prev is not None: output.bkg_prev = bkg_prev.reshape(-1, 3)
            if norm_prev is not None: output.norm_prev = norm_prev.reshape(-1, 3)
            if diffrens_prev is not None:
                for param in self.diffren_params:
                    if diffrens_prev[param].ndim == 3 and diffrens_prev[param].shape[-1] == 3:
                        output[f"{param}_prev"] = diffrens_prev[param].reshape(-1, 3)
                    else:
                        output[f"{param}_prev"] = diffrens_prev[param].reshape(-1, 1)
            if sky_masks_prev is not None: output.sky_masks_prev = sky_masks_prev.reshape(-1, 1)

        if rgb_next is not None:
            output.rgb_next = rgb_next.reshape(-1, 3)
            output.msk_next = msk_next.reshape(-1, 1)
            output.wet_next = wet_next.reshape(-1, 1)
            if dpt_next is not None: output.dpt_next = dpt_next.reshape(-1, 1)
            if bkg_next is not None: output.bkg_next = bkg_next.reshape(-1, 3)
            if norm_next is not None: output.norm_next = norm_next.reshape(-1, 3)
            if diffrens_next is not None:
                for param in self.diffren_params:
                    if diffrens_next[param].ndim == 3 and diffrens_next[param].shape[-1] == 3:
                        output[f"{param}_next"] = diffrens_next[param].reshape(-1, 3)
                    else:
                        output[f"{param}_next"] = diffrens_next[param].reshape(-1, 1)
            if sky_masks_next is not None: output.sky_masks_next = sky_masks_next.reshape(-1, 1)

        if should_crop_ixt:
            # Prepare the resized ixts
            self.crop_ixts(output, x, y, w, h)
            self.crop_ixts(output_neighbors_prev, x, y, w, h) if output_neighbors_prev is not None else None
            self.crop_ixts(output_neighbors_next, x, y, w, h) if output_neighbors_next is not None else None
            
            output.meta_prev.H, output.meta_prev.W = output_neighbors_prev.H, output_neighbors_prev.W if output_neighbors_prev is not None else (None, None)
            output.meta_prev.K, output.meta_prev.R, output.meta_prev.T = (output_neighbors_prev.K, output_neighbors_prev.R, output_neighbors_prev.T) if output_neighbors_prev is not None else (None, None, None)
            output.meta_next.H, output.meta_next.W = output_neighbors_next.H, output_neighbors_next.W if output_neighbors_next is not None else (None, None)
            output.meta_next.K, output.meta_next.R, output.meta_next.T = (output_neighbors_next.K, output_neighbors_next.R, output_neighbors_next.T) if output_neighbors_next is not None else (None, None, None)
            
        elif should_sample_patch:
            # Prepare the full sampling output
            H, W = output.H, output.W
            K, R, T = output.K, output.R, output.T
            K_prev, R_prev, T_prev = None, None, None
            if output_neighbors_prev is not None:
                K_prev, R_prev, T_prev = output_neighbors_prev.K, output_neighbors_prev.R, output_neighbors_prev.T
            K_next, R_next, T_next = None, None, None
            if output_neighbors_next is not None:
                K_next, R_next, T_next = output_neighbors_next.K, output_neighbors_next.R, output_neighbors_next.T
            
            output.meta_prev = dotdict()
            output.meta_next = dotdict()
            if output_neighbors_prev is not None:
                output.meta_prev.H, output.meta_prev.W = output_neighbors_prev.H, output_neighbors_prev.W
                output.meta_prev.K, output.meta_prev.R, output.meta_prev.T = K_prev, R_prev, T_prev
            if output_neighbors_next is not None:
                output.meta_next.H, output.meta_next.W = output_neighbors_next.H, output_neighbors_next.W
                output.meta_next.K, output.meta_next.R, output.meta_next.T = K_next, R_next, T_next
    ...
    def __getitem__(self, index: int):  # for now, we are using the dict as list
        ...
        if 'diffrens' in output:
            diffrens = dotdict()
            for param in self.diffren_params:
                diffrens[param] = output[param].view(H, W, -1)
        if 'sky_masks' in output:  # sky mask is not used for sampling, but for visualization
            sky_masks = output.sky_masks.view(H, W, 1)  # (H, W, 1)
        ...
        if 'diffrens' in output:
            for param in self.diffren_params:
                diffrens[param] = diffrens[param][i, j]
        if 'sky_masks' in output: sky_masks = sky_masks[i, j]
        ...
        if 'diffrens' in output:
            for param in self.diffren_params:
                output[param] = diffrens[param]
        if 'sky_masks' in output: output.sky_masks = sky_masks
