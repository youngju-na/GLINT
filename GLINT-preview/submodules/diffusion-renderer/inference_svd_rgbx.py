            ...
            for chunk_ind in range(len(input_image_relative_path_chunks)):
                ...
                for ind in range(inference_n_frames):
                    ...
                    if cfg.get('save_image', False):
                        ...
                        # 프레임 순서 조정이 적용된 경우 원래 프레임 번호를 계산
                        if frame_reorder_mode == "move_first_50_to_end":
                            ...
                            if total_frames > 50:
                                if current_global_index < total_frames - 50:
                                    # 50번째 이후 프레임들 (원래 위치로 복원)
                                    original_frame_index = current_global_index + 50
                                else:
                                    # 처음 50개 프레임들 (마지막으로 이동된 것들)
                                    original_frame_index = current_global_index - (total_frames - 50)
                            else:
                                original_frame_index = current_global_index
                            
                            save_path = os.path.join(inference_save_dir, f"{video_relative_base_name}/{chunk_ind:04d}.frame_{original_frame_index:04d}.rgb.png")
                        else:
                            save_path = os.path.join(inference_save_dir, f"{video_relative_base_name}/{chunk_ind:04d}.{original_basename}.rgb.png")
                    ...
                    if cfg.get('save_image', False):
                        for ind in range(len(inference_image_list)):
                            ...
                            if frame_reorder_mode == "move_first_50_to_end":
                                ...
                                save_path = os.path.join(inference_save_dir, f"{video_relative_base_name}/{chunk_ind:04d}.frame_{original_frame_index:04d}.{inference_pass}.png")
                            else:
                                save_path = os.path.join(inference_save_dir, f"{video_relative_base_name}/{chunk_ind:04d}.{original_basename}.{inference_pass}.png")
