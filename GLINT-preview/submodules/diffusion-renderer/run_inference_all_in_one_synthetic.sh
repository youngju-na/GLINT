...
# ====== 기본 경로 설정 ======
ORIGINAL_BASE_DIR="/home2/guest/datasets/nerfstudio/manual_synthetic_GT_cam"
RESIZED_BASE_DIR="/home2/guest/datasets/nerfstudio/manual_synthetic_GT_cam_resize"
INFERENCE_BASE_SAVE_DIR="/home2/guest/datasets/nerfstudio/manual_synthetic_GT_cam_inference_all_no_resize"
...
find "${BASE_DIR}" -maxdepth 1 -mindepth 1 -type d -printf '%f\n' | while IFS= read -r scene_name; do
    ...
    CUDA_VISIBLE_DEVICES=${CURRENT_GPU} python inference_svd_rgbx.py \
            ...
            model_passes="['basecolor','normal','depth','diffuse_albedo','roughness','metallic']" \
            chunk_mode="all" &
