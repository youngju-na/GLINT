...
class Front3DPointInRoomSampler:
    ...
    def get_floor_areas(self):
        areas = []
        for fl in self.used_floors:
            bbox = fl.get_bound_box()
            area = np.cross(bbox[4]-bbox[0], bbox[3]-bbox[0])
            area = np.linalg.norm(area)
            areas.append(area)
        return areas

    def sample_by_floor_id(self, height: float, max_tries: int = 1000, floor_id: int = 0) -> np.ndarray:
        ...
        :param floor_id: sample cam location on the i-th floor.
        :return: The sampled point.
        """
        for _ in range(max_tries):
            # Sample room via floor objects
            floor_obj = self.used_floors[floor_id]
