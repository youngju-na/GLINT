...
def add_indoor_lights(fmin, fmax, ceil_z, room_center, strength_scale=1.0):
    # 천장 면에서 균일한 AREA 라이트 그리드 (자연스러운 전역조명)
    nx, ny = 2, 2  # 그리드 개수 (필요하면 3x2로 늘리세요)
    xs = np.linspace(fmin[0]+0.2, fmax[0]-0.2, nx)
    ys = np.linspace(fmin[1]+0.2, fmax[1]-0.2, ny)
    for x in xs:
        for y in ys:
            L = bproc.types.Light(type="AREA")
            L.set_location([float(x), float(y), float(ceil_z - 0.15)])
            L.set_rotation_euler([0.0, 0.0, 0.0])  # 아래로 비추도록 기본값
            L.set_energy(2500.0 * strength_scale)
            # bproc.types.Light does not expose set_size; set Blender light data size directly
            try:
                L.blender_obj.data.size = 1.6
            except Exception:
                pass

    # 벽면 보조광(soft fill) : 큰 벽을 따라 부드럽게
    # 좌/우/앞/뒤 벽 가운데에 SOFT AREA 라이트 배치
    # wall_centers = [
    #     [(fmin[0], 0.5*(fmin[1]+fmax[1]), 1.2), [0, np.pi/2, 0]],    # 왼쪽 벽
    #     [(fmax[0], 0.5*(fmin[1]+fmax[1]), 1.2), [0, -np.pi/2, 0]],   # 오른쪽 벽
    #     [(0.5*(fmin[0]+fmax[0]), fmin[1], 1.2), [0, 0, 0]],          # 앞 벽
    #     [(0.5*(fmin[0]+fmax[0]), fmax[1], 1.2), [0, np.pi, 0]],      # 뒤 벽
    # ]
    # for loc, rot in wall_centers:
    #     L = bproc.types.Light(type="AREA")
    #     L.set_location([float(loc[0]), float(loc[1]), float(loc[2])])
    #     L.set_rotation_euler(rot)
    #     L.set_energy(1200.0 * strength_scale)
    #     # set size directly on Blender light data
    #     try:
    #         L.blender_obj.data.size = 1.2
    #     except Exception:
    #         # fallback for rectangular area lights: try size_x/size_y
    #         try:
    #             L.blender_obj.data.size_x = 1.2
    #             L.blender_obj.data.size_y = 1.2
    #         except Exception:
    #             pass

    # 약한 SUN으로 방향성 강조(창문 유사 방향: 방 중심→최대 y쪽)
    sun = bproc.types.Light(type="SUN")
    sun.set_energy(2.0 * strength_scale)
    # y가 큰 쪽에서 들어오는 느낌
    sun.set_rotation_euler([0.9, 0.0, -1.2])