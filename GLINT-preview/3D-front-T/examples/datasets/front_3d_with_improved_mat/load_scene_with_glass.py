...
def create_glass_container_with_duck(location, duck_model_path):
    """Loads a duck model, encloses it in a glass container, and parents them."""
    import bpy
    ...
    # 2) Calculate duck's world-space bounding box to determine container size
    all_corners = []
    for obj in loaded_duck_objects:
        # Use a copy of the bounding box to avoid modifying the original
    ...
    if not all_corners:
        # If duck has no geometry, create a default-sized container
        duck_size = np.array([1.0, 1.0, 1.0])
        duck_center = np.array([0.0, 0.0, 0.0])
    else:
    ...
    padding = 0.5
    container_dims = duck_size + padding
    ...
    # 4) Move duck objects to the container's location
    offset = location - duck_center
    for obj in loaded_duck_objects:
        obj.set_location(obj.get_location() + offset)

    # 5) Configure glass container (solidify, materials)
    solid = glass_container.blender_obj.modifiers.new(name="Solidify", type='SOLIDIFY')
    ...
    edge_thickness = 0.08  # Increase thickness for a more prominent frame

    frame_edges = []
    def add_edge(pos, scale):
        e = bproc.object.create_primitive('CUBE', location=pos, scale=scale)
    ...
    for ys in (-1, 1):
        for zs in (-1, 1):
            pos = [location[0], location[1] + ys * (half_y - edge_thickness/2.0), location[2] + zs * (half_z - edge_thickness/2.0)]
            scale = [half_x - edge_thickness/2.0, edge_thickness/2.0, edge_thickness/2.0]
            add_edge(pos, scale)
    # Y-direction edges
    for xs in (-1, 1):
        for zs in (-1, 1):
            pos = [location[0] + xs * (half_x - edge_thickness/2.0), location[1], location[2] + zs * (half_z - edge_thickness/2.0)]
            scale = [edge_thickness/2.0, half_y - edge_thickness/2.0, edge_thickness/2.0]
            add_edge(pos, scale)
    # Z-direction edges
    for xs in (-1, 1):
        for ys in (-1, 1):
            pos = [location[0] + xs * (half_x - edge_thickness/2.0), location[1] + ys * (half_y - edge_thickness/2.0), location[2]]
            scale = [edge_thickness/2.0, edge_thickness/2.0, half_z - edge_thickness/2.0]
            add_edge(pos, scale)
...
if __name__ == '__main__':
    ...
    container_location = find_scene_center_and_surface(loaded_objects)
    ...
    if all_container_objects:
        ...
        if main_container:
            ...
            # Parent all other container parts to the main container
            for obj in all_container_objects:
                if obj != main_container:
                    main_container.add_child(obj)
        else:
